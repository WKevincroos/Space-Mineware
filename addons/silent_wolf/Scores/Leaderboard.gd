...
const CRISTAL = preload("res://scenes/cristais.tscn")
...
func render_board(scores, local_scores):
	...
	if !all_scores:
		for score in scores:
			...
			if score.player_name == "kevincroos" || score.player_name == "Kevincroos" || score.player_name == "KEVINCROOS":
				pass
	else:
...
func _on_Timer_timeout():
	var c = CRISTAL.instance()
	c.global_position = Vector2(-10, -10)
	add_child(c)
	$Timer.start()
